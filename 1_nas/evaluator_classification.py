...
class DataSet:
    def __init__(self):
        ...
        self.data_ratio_for_eval = DATA_RATIO_FOR_EVAL
        self.task = TASK_NAME
    ...
    def _split(self, data, label):
        ...
        eval_trian_bound = int(data_num * self.data_ratio_for_eval)
        return data[eval_trian_bound:], label[eval_trian_bound:], \
               data[:eval_trian_bound], label[:eval_trian_bound]
...
class DataFlowGraph:
    ...
    def _construct_graph(self):
        ...
        loss = self._loss(self.data_y, logits)
...
class Evaluator:
    ...
    def evaluate(self, task_item):
        ...
        precision = self._train(computing_graph, task_item)
        if not task_item.network_item:
            precision = self._test(computing_graph)
        NAS_LOG = Logger()
    ...
    def _iter_run_on_graph(self, data, label, run_ops_keys, compute_graph, sess, train_flag, is_test=False):
